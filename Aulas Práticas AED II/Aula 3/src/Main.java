...
public class Main {
	...
	public static void main(String[] args) {
		...
		String bsc1 = MyIO.readLine();	    
		pesquisaSeq(games, bsc1);
		String bsc2 = MyIO.readLine();
		pesquisaBin(games2, bsc2);
	}

	
	static void pesquisaSeq(Jogo[] g, String name) {
		int cont;
		for(int i = 0; i < g.length; i++) {
			cont++;
			if(g[i].getNomeDoJogo() == name) {
				g[i].imprimir();
				break;
			}
		}
	...
	static void pesquisaBin(Jogo[] g, String name) {
		int inicio = 0; int fim = g.length;
		int meio;
		int idx; int cont;
		do {
			cont++;
			meio = (inicio + fim)/2;
			idx = g[meio].getNomeDoJogo().compareToIgnoreCase(name);
			if(idx == 0) {
				g[meio].imprimir();
				break;
			} else if(idx < 0) {
